...
/*				of a form
/* dependency: 	jQuery.confirm, wl_Number*, wl_Slider*, wl_Date*, wl_Value*,
/* 				wl_Password*, wl_File*, wl_Multiselect*
/*				* only when fields are within the form
/*----------------------------------------------------------------------*/
jQuery.fn.wl_Form = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Form.methods[method]) {
			return jQuery.fn.wl_Form.methods[method].apply(this, Array.prototype.slice.call(args, 1));
		} else if (typeof method === 'object' || !method) {
			if ($this.data('wl_Form')) {
				var opts = jQuery.extend({}, $this.data('wl_Form'), method);
			} else {
				var opts = jQuery.extend({}, jQuery.fn.wl_Form.defaults, method, $this.data());
			}
		} else {
		...
		if (!$this.data('wl_Form')) {
			...
			$submitbtn.bind('click.wl_Form', function (event) {
				jQuery.fn.wl_Form.methods.submit.call($this[0]);
				return false;
			});
			//bind the click action to the submit button
			$resetbtn.bind('click.wl_Form', function (event) {
				jQuery.fn.wl_Form.methods.reset.call($this[0]);
				return false;
			});
		...
		if (opts) jQuery.extend($this.data('wl_Form'), opts);
	});

};

jQuery.fn.wl_Form.defaults = {
...
jQuery.fn.wl_Form.version = '1.3.4';


jQuery.fn.wl_Form.methods = {
	disable: function () {
		var $this = jQuery(this),
	...
	reset: function () {
		...
		_inputs.each(function () {
			...
			if (_type == "checkbox") {
				...
				if (!_this.data('wl_Slider').connect) {
					if (_this.data('wl_Slider').range) {
						_this.slider('option', 'values', jQuery.parseData(_this.data('wl_initdata')));
					} else {
	...
	submit: function () {
		...
		var $this = jQuery(this),
			...
			_callbackReturn, _addHiddenField = function (after, id, name, value) {
				if (!jQuery('#' + id).length) jQuery('<input>', {
					type: 'hidden',
					id: id,
					name: name,
					value: value
		...
		_inputs.each(function (i, e) {
			...
			if (_this.data('required')) {
				if ((!_this.val() || _this.is(':checkbox') && !_this.is(':checked')) && !_this.data('wl_File')) {
					...
					var checked = jQuery('input[name='+_this.attr('name')+']:checked');
			...
			if (_this.data('wl_File') && !jQuery.isEmptyObject(_this.data('wl_File').queue)) {
				//use the filepool for the tooltip
			...
			if (_this.data('wl_Password')) {
				...
				if ((_this.data('wl_Password').confirm && _this.data('wl_Password').connect && value != jQuery('#' + _this.data('wl_Password').connect).val()) || (value && value.length < _this.data('wl_Password').minLength)) {
					//_row.addClass('error');
		...
		if (!_submit) {
			...
			jQuery.each(_requiredelements, function (i, e) {
				...
				e.tipsy(jQuery.extend({}, config.tooltip, _opts.tooltip, {
					trigger: 'manual',
				...
				if(e.is(':radio')){
					var radiosiblings = jQuery('input[name='+e.attr('name')+']');
					...
					e.bind('focus.tooltip, click.tooltip, change.tooltip', function () {
						jQuery(this).unbind('focus.tooltip, click.tooltip, change.tooltip').tipsy('hide');
					});
				}
			});
			...
			jQuery.each(_validelements, function (i, e) {
				...
				e.bind('focus.tooltip, click.tooltip', function () {
					jQuery(this).unbind('focus.tooltip, click.tooltip').tipsy('hide');
				});
			});

			//iterate through all password fields
			jQuery.each(_passwordelements, function (i, e) {
				var text = '',
					value = e.val(),
				...
				if (e.data('wl_Password').confirm) {
					var connect = jQuery('#' + e.data('wl_Password').connect);

					//but password is not equal confimration
					if (value != connect.val()) {

						//tipsy on the confirmation field
						connect.tipsy(jQuery.extend({}, config.tooltip, _opts.tooltip, {
							trigger: 'manual',
			...
			jQuery.each(_fileelements, function (i, e) {
		...
		if (_opts.confirmSend && _opts.sent === true) {
			jQuery.confirm(_opts.text.sendagain, function () {
				_opts.sent = false;
				jQuery.fn.wl_Form.methods.submit.call($this[0]);
			});
			return false;
		...
		_inputs.each(function (i, e) {
			...
			if (_el.data('wl_Date')) {
				...
				var connect = $this.find('input[data-connect=' + e.id + ']').eq(0),
					...
					//format: YYYY-MM-DD
					date = dateobj.getFullYear() + '-' + jQuery.leadingZero(dateobj.getMonth() + 1) + '-' + jQuery.leadingZero(dateobj.getDate());
				if (dateobj.getTime()) {
				...
				if (!_el.data('wl_Slider').connect) {
					...
					if (!_opts.ajax) {
						if (_el.data('wl_Slider').range !== true) {
							var input = jQuery('#' + _el.data('wl_Slider').connect);
							if (!input.attr('name')) input.attr('name', _el.data('wl_Slider').connect);
						} else {
							var connect = jQuery.parseData(_el.data('wl_Slider').connect, true);
							var input1 = jQuery('#' + connect[0]);
							var input2 = jQuery('#' + connect[1]);
				...
				if (jQuery.isEmptyObject(_data[key])) {
					_data[key] = null;
					//insert a hidden field for non ajax submit
				...
				if (/\[\]$/.test(key) && !jQuery.isArray(val)) {
					_data[key] = _data[key] || [];
					_data[key].push(val);
				} else {
					_data[key] = val;
		...
		//now disable it
		jQuery.fn.wl_Form.methods.disable.call(this);
		...
		jQuery.ajax({
			...
			complete: function (jqXHR, textStatus) {
				jQuery.fn.wl_Form.methods.enable.call($this[0]);
				_opts.sent = true;
	...
	unserialize: function (string) {
		...
		if (_searchquery) {
			...
			try {
				...
				$this.find("input,textarea,select,div.date,div.slider").each(function () {
					...
					if (values[tag_name] != null) {
						...
						if (_type == "checkbox") {
							...
							jQuery('input[id="' + values[tag_name] + '"]').data('wl_initdata', true).attr("checked", true);
							...
							if (/(\d\d:\d\d)$/.test(values[tag_name])) {
								...
								jQuery('input[data-connect="' + tag_name + '"]').data('wl_initdata', time).val(time).data('wl_Time').time = time;
							} else {
							...
							if (!_this.data('wl_Slider').connect) {
								if (_this.data('wl_Slider').range) {
									_this.slider('option', 'values', jQuery.parseData(values[tag_name]));
								} else {
				...
				jQuery.msg($this.data('wl_Form').text.parseerror.replace('%e', e));
			}
		}
	},
	set: function () {
		...
		jQuery.each(options, function (key, value) {
			if (jQuery.fn.wl_Form.defaults[key] !== undefined && jQuery.fn.wl_Form.defaults[key] !== null) {

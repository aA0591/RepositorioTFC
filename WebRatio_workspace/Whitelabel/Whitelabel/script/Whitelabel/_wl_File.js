	...
	return this.each(function () {
		...
		function refreshFileList(){
			...
			opts.filepool.find('li').each(function(){
				...
				if(data.isValidated){
					files.push(jQuery(this).find('.name').text());
				}
			});
       ...
       function startHandler (e) {
			...
			opts.filepool.find('li').each(function(){
				...
				if(data && data.isValidated && data.submit && !data.jqXHR){
               		jQuery(this).data('data').jqXHR = data.submit();
				}
			});
        }
		...
		function hasError (file) {
            ...
            if (false && opts.allowedExtensions && jQuery.inArray(file.ext, opts.allowedExtensions) == -1) {
				return {

...
jQuery.fn.wl_Valid = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Valid.methods[method]) {
			return jQuery.fn.wl_Valid.methods[method].apply(this, Array.prototype.slice.call(args, 1));
		} else if (typeof method === 'object' || !method) {
			if ($this.data('wl_Valid')) {
				var opts = jQuery.extend({}, $this.data('wl_Valid'), method);
			} else {
				var opts = jQuery.extend({}, jQuery.fn.wl_Valid.defaults, method, $this.data());
			}
		} else {
		...
		if (!$this.data('wl_Valid')) {
			...
			$this.bind({
				...
				'change.wl_Valid': function () {
					...
					jQuery.fn.wl_Valid.methods.validate.call($this[0]);
		...
		if (opts) jQuery.extend($this.data('wl_Valid'), opts);
		jQuery.fn.wl_Valid.methods.validate.call($this);
	});

};

jQuery.fn.wl_Valid.defaults = {
	errorClass: 'error',
	instant: true,
	regex: /.*/,
	minLength: 0,
...
jQuery.fn.wl_Valid.version = '1.0';


jQuery.fn.wl_Valid.methods = {
	validate: function () {
		var $this = jQuery(this),
	...
	set: function () {
		...
		jQuery.each(options, function (key, value) {
			if (jQuery.fn.wl_Valid.defaults[key] !== undefined || jQuery.fn.wl_Valid.defaults[key] == null) {
				switch (key) {
				case 'regex':
...
jQuery.fn.wl_Mail = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Valid.methods[method]) {
			...
			if ($this.data('wl_Valid')) {
				...
				var opts = jQuery.extend({}, jQuery.fn.wl_Mail.defaults, method, $this.data());
			}
		} else {
			jQuery.error('Method "' + method + '" does not exist');
		}
...
jQuery.fn.wl_URL = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Valid.methods[method]) {
			...
			if ($this.data('wl_Valid')) {
				...
				var opts = jQuery.extend({}, jQuery.fn.wl_URL.defaults, method, $this.data());
			}
		} else {
			jQuery.error('Method "' + method + '" does not exist');
		}

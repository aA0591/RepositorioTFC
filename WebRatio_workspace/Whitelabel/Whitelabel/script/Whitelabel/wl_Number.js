...
jQuery.fn.wl_Number = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Number.methods[method]) {
			return jQuery.fn.wl_Number.methods[method].apply(this, Array.prototype.slice.call(args, 1));
		} else if (typeof method === 'object' || !method) {
			if ($this.data('wl_Number')) {
				var opts = jQuery.extend({}, $this.data('wl_Number'), method);
			} else {
				var opts = jQuery.extend({}, jQuery.fn.wl_Number.defaults, method, $this.data());
			}
		} else {
			...
			$this.bind({
				...
				'mousewheel.wl_Number': function (event, delta) {
					...
					if (opts.mousewheel) {
						...
						jQuery.fn.wl_Number.methods.change.call($this[0], delta);
					}
				},
				...
				'change.wl_Number': function () {
					...
					//correct the input
					jQuery.fn.wl_Number.methods.correct.call($this[0]);
					//callback
		...
		if (opts) jQuery.extend($this.data('wl_Number'), opts);
	});

};

jQuery.fn.wl_Number.defaults = {
	step: 1,
	decimals: 0,
	start: 0,
	min: null,
...
jQuery.fn.wl_Number.version = '1.0';


jQuery.fn.wl_Number.methods = {
	correct: function () {
		var $this = jQuery(this),
			//replace ',' with '.' because in some countries comma is the separator
			val = $this.val().replace(/,/g, '.');
		if (val) jQuery.fn.wl_Number.methods.printValue.call(this, parseFloat(val));
	},
	change: function (delta) {
		...
		jQuery.fn.wl_Number.methods.printValue.call(this, _new);
	...
	set: function () {
		...
		jQuery.each(options, function (key, value) {
			if (jQuery.fn.wl_Number.defaults[key] !== undefined || jQuery.fn.wl_Number.defaults[key] == null) {

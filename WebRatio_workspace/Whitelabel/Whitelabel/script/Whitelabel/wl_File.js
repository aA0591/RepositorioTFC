...
jQuery.fn.wl_File = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_File.methods[method]) {
			return jQuery.fn.wl_File.methods[method].apply(this, Array.prototype.slice.call(args, 1));
		} else if (typeof method === 'object' || !method) {
			if ($this.data('wl_File')) {
				var opts = jQuery.extend({}, $this.data('wl_File'), method);
			} else {
				var opts = jQuery.extend({}, jQuery.fn.wl_File.defaults, method, $this.data());
			}
		} else {
			try {
		...
		if (!$this.data('wl_File')) {
			...
			if (typeof opts.allowedExtensions === 'string') opts.allowedExtensions = jQuery.parseData(opts.allowedExtensions);
			...
			opts.uiCancel = jQuery('<a>', {
				...
				jQuery.each(queue, function (name) {
					if (queue[name]) {
				...
				opts.onDelete.call($this[0], jQuery.map(el,function(k,v){return jQuery(k).data('fileName');}));
			});


			//filepool and dropzone
			opts.filepool = jQuery('<ul>', {
			...
			.delegate('a.cancel', 'click', function () {
				...
				$this.data('wl_File').files = files = jQuery.map(files, function (filename) {
				...
				send: function (e, data) {
					...
					jQuery.each(data.files, function (i, file) {
						if (queue[file.name]) {
							...
							if (jQuery.inArray(file.name, files) == -1) {
		...
		function addFile(file, data) {
			var name = file.name;
			var html = jQuery('<li><span class="name">' + name + '</span><span class="progress"></span><span class="status">' + opts.text.ready + '</span><a class="cancel" title="' + opts.text.cancel + '">' + opts.text.cancel + '</a></li>').data('fileName', name).appendTo(opts.filepool);
			queue[name] = {
				element: html,
				data: data,
		...
		function getError(file) {
			...
			if (opts.allowedExtensions && jQuery.inArray(file.ext, opts.allowedExtensions) == -1) {
				return {
		...
		if (opts) jQuery.extend($this.data('wl_File'), opts);

	});

};

jQuery.fn.wl_File.defaults = {
	url: 'upload.php',
	autoUpload: true,
	paramName: 'files',
	multiple: false,
...
jQuery.fn.wl_File.version = '1.2';


jQuery.fn.wl_File.methods = {
	reset: function () {
		var $this = jQuery(this),
			opts = $this.data('wl_File');
		
		//default value if uniform is used
		if(jQuery.uniform)$this.next().html(jQuery.uniform.options.fileDefaultText);
		//empty file pool
		opts.filepool.empty();
		//reset button
	...
	set: function () {
		...
		jQuery.each(options, function (key, value) {
			if (jQuery.fn.wl_File.defaults[key] !== undefined || jQuery.fn.wl_File.defaults[key] == null) {

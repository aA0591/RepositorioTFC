...
jQuery.fn.wl_Multiselect = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Multiselect.methods[method]) {
			return jQuery.fn.wl_Multiselect.methods[method].apply(this, Array.prototype.slice.call(args, 1));
		} else if (typeof method === 'object' || !method) {
			if ($this.data('wl_Multiselect')) {
				var opts = jQuery.extend({}, $this.data('wl_Multiselect'), method);
			} else {
				var opts = jQuery.extend({}, jQuery.fn.wl_Multiselect.defaults, method, $this.data());
			}
		} else {
		...
		if (!$this.data('wl_Multiselect')) {
			...
			jQuery('<div class="comboselectbox"><div class="combowrap"><ul class="comboselect"></ul></div><div class="comboselectbuttons"><a class="add btn"></a><a class="addall btn"></a><a class="removeall btn"></a><a class="remove btn"></a></div><div class="combowrap"><ul class="comboselect"></ul></div></div>').insertAfter($this);
		...
		if (opts.items.length) {
			if (opts.selected.length && !jQuery.isArray(opts.selected)) opts.selected = [opts.selected];
			jQuery.each(opts.items, function (i, data) {
				...
				if (opts.selected.length && jQuery.inArray(value, opts.selected) != -1) selected = ' selected';
				$i = $i.add(jQuery('<option value="' + value + '"' + selected + '>' + name + '</option>'));
			});
			$i.appendTo($this);
		}
		...
		jQuery.each($this.find('option'), function (i, e) {
			var _this = jQuery(this),
				name = _this.text(),
				value = _this.val();
			var item = jQuery('<li><a>' + name + '</a></li>').append('<a class="add"></a>').data({
				'pos': i,
				'value': value,
				'name': name,
				'native': _this
		...
		$add.bind('click.wl_Multiselect', function () {
			var selection = jQuery.map(opts.pool.find('li.selected'), function (el) {
		...
		$remove.bind('click.wl_Multiselect', function () {
			var selection = jQuery.map(opts.selection.find('li.selected'), function (el) {
				return jQuery(el).data('value');
			});
		...
		$addall.bind('click.wl_Multiselect', function () {
			var selection = jQuery.map(opts.pool.find('li'), function (el) {
				return jQuery(el).data('value');
			});
			$this.wl_Multiselect('select', selection);
		});
		...
		$removeall.bind('click.wl_Multiselect', function () {
			var selection = jQuery.map(opts.selection.find('li'), function (el) {
				return jQuery(el).data('value');
			});
			$this.wl_Multiselect('unselect', selection);
		});
		...
		opts.pool.delegate('li', 'click.wl_Multiselect', {
			...
			$this.wl_Multiselect('select', jQuery(this).data('value'));
		}).delegate('a.add', 'click.wl_Multiselect', function () {
			$this.wl_Multiselect('select', jQuery(this).parent().data('value'));
		}).disableSelection();

		opts.selection.delegate('li', 'click.wl_Multiselect', {
			'list': opts.selection
		}, clickHandler).delegate('a.remove', 'click.wl_Multiselect', function () {
			$this.wl_Multiselect('unselect', jQuery(this).parent().data('value'));
		});
		...
		if (opts) jQuery.extend($this.data('wl_Multiselect'), opts);

	});

};

jQuery.fn.wl_Multiselect.defaults = {
	height: 200,
	items: [],
	selected: [],
	showUsed: false,
...
jQuery.fn.wl_Multiselect.version = '1.2';


jQuery.fn.wl_Multiselect.methods = {
	add: function (items, select) {
		...
		jQuery.each(_items, function (value, name) {
			//make native items
			var _native = jQuery('<option value="' + value + '">' + name + '</option>').appendTo($this);
			//and elements
			var item = jQuery('<li><a>' + name + '</a></li>').append('<a class="add"></a>').data({
				'pos': i++,
				'native': _native,
		...
		opts.onAdd.call($this[0], jQuery.map(_items, function (k, v) {
			return k;
		}));
	},
	...
	set: function () {
		...
		jQuery.each(options, function (key, value) {
			if (jQuery.fn.wl_Multiselect.defaults[key] !== undefined) {

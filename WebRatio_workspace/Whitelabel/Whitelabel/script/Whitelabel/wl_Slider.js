...
jQuery.fn.wl_Slider = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Slider.methods[method]) {
			return jQuery.fn.wl_Slider.methods[method].apply(this, Array.prototype.slice.call(args, 1));
		} else if (typeof method === 'object' || !method) {
			if ($this.data('wl_Slider')) {
				var opts = jQuery.extend({}, $this.data('wl_Slider'), method);
			} else {
				var opts = jQuery.extend({}, jQuery.fn.wl_Slider.defaults, method, $this.data());
			}
		} else {
			try {
			...
			$this.bind('mousewheel.wl_Slider', function (event, delta) {
				if (opts.mousewheel) {
					...
					if ($this.data('range') !== true) {
						...
						jQuery.fn.wl_Slider.methods.values.call($this[0], _value + (delta * opts.step));
					...
					//slider with two handlers
						...
						if (opts.orientation == 'horizontal') {
							if (_h1.left + (_h2.left - _h1.left) / 2 > event.clientX) {
								jQuery.fn.wl_Slider.methods.values.call($this[0], Math.min(_values[0] + (delta * opts.step), _values[1]), _values[1]);
							} else {
								jQuery.fn.wl_Slider.methods.values.call($this[0], _values[0], Math.max(_values[1] + (delta * opts.step), _values[0]));
							}
					...
					jQuery.fn.wl_Slider.methods.slide.call($this[0]);
				}
			});
		} else {
			//destroy it
		...
		$this.slider(opts).bind('slide', function (event, ui) {
			jQuery.fn.wl_Slider.methods.slide.call($this[0], ui.value);
		}).bind('slidechange', function (event, ui) {
			jQuery.fn.wl_Slider.methods.change.call($this[0], ui.value);
		});
		...
		if (opts.connect) {
			...
			//single slider
			if ($this.data('range') !== true) {
				var _input = jQuery('#' + opts.connect),
				...
				_input.val(_value).wl_Number('set', jQuery.extend({}, opts, {
			...
			//range slider with two handlers
				...
				var _input = jQuery.parseData(opts.connect, true),
					_input1 = jQuery('#' + _input[0]),
					_input2 = jQuery('#' + _input[1]),
				...
				$this.unbind('slide slidechange').slider('option', 'values', [_value1, _value2]).bind('slide', function (event, ui) {
					...
					jQuery.fn.wl_Slider.methods.slide.call($this[0], ui.values);
				}).bind('slidechange', function (event, ui) {
					_input1.val(ui.values[0]);
					_input2.val(ui.values[1]);
					jQuery.fn.wl_Slider.methods.change.call($this[0], ui.values);
				});

				//set callbacks to the connected input fields
				_input1.wl_Number('set', jQuery.extend({}, opts, {
					onChange: function (value) {
				...
				_input2.wl_Number('set', jQuery.extend({}, opts, {
					onChange: function (value) {
		...
		if(opts.tooltip){
			...
			//set the gravity
			if(jQuery.isArray(opts.tooltipGravity)){
				if(opts.orientation == 'horizontal'){
			...
			//for each handle (e)
			jQuery.each(e, function(i,handle){
				
				var value;
				
				handles.push(jQuery(handle));
				...
				handles[i].tipsy(jQuery.extend({}, config.tooltip, {
					trigger: 'manual',
					gravity: gravity,
					html: true,
			...
			.bind('slidestart', function (event, ui) {
				jQuery(ui.handle).unbind('mouseleave.wl_Slider touchstart.wl_Slider mouseenter.wl_Slider touchend.wl_Slider');
			})
		...
		if (opts) jQuery.extend($this.data('wl_Slider'), opts);
	});

};

jQuery.fn.wl_Slider.defaults = {
	min: 0,
	max: 100,
	step: 1,
	animate: false,
...
jQuery.fn.wl_Slider.version = '1.1.1';


jQuery.fn.wl_Slider.methods = {
	disable: function () {
		var $this = jQuery(this),
		...
		if (opts.connect) {
			if ($this.data('range') !== true) {
				jQuery('#' + opts.connect).prop('disabled', true);
			} else {
				var _input = jQuery.parseData(opts.connect, true),
					_input1 = jQuery('#' + _input[0]),
					_input2 = jQuery('#' + _input[1]);
	...
	enable: function () {
		...
		if ($this.data('wl_Slider').connect) {
			if ($this.data('range') !== true) {
				jQuery('#' + opts.connect).prop('disabled', false);
			} else {
				var _input = jQuery.parseData(opts.connect, true),
					_input1 = jQuery('#' + _input[0]),
					_input2 = jQuery('#' + _input[1]);
	...
	values: function () {
		...
		if (opts.range === true) {
			...
			jQuery.fn.wl_Slider.methods.value.call(this, arguments[0]);
		}
	},
	set: function () {
		var $this = jQuery(this),
		...
		if ($this.data('wl_Slider').connect) {
			if ($this.data('range') !== true) {
				var _input1 = jQuery('#' + $this.data('wl_Slider').connect);
			} else {
				var _input = jQuery.parseData($this.data('wl_Slider').connect, true),
					_input1 = jQuery('#' + _input[0]),
					_input2 = jQuery('#' + _input[1]);
			}
		}
		jQuery.each(options, function (key, value) {
			if (jQuery.fn.wl_Slider.defaults[key] !== undefined || jQuery.fn.wl_Slider.defaults[key] == null) {

...
/*----------------------------------------------------------------------*/
jQuery.fn.wl_Chart = function (method) {
	...
	return this.each(function () {
		...
		if (jQuery.fn.wl_Chart.methods[method]) {
			return jQuery.fn.wl_Chart.methods[method].apply(this, Array.prototype.slice.call(args, 1));
		} else if (typeof method === 'object' || !method) {
			if ($this.data('wl_Chart')) {
				var opts = jQuery.extend({}, $this.data('wl_Chart'), method);
			} else {
				var opts = jQuery.extend({}, jQuery.fn.wl_Chart.defaults, method, $this.data());
			}
		} else {
			try {
		...
		if (!/^(lines|bars|pie)$/.test(opts.type)) {
			jQuery.error('Type of "' + opts.type + '" is not allowed');
		}
		...
		if (opts.tooltip) {
			...
			opts.holder.tipsy(jQuery.extend({}, config.tooltip, {
				fallback: '',
				followMouse: true,
			...
			opts.holder.bind("plothover", function (e, pos, item) {
				if (item) {
					...
					if (item.datapoint.toString() != prev) {
						...
						if (jQuery.isFunction(opts.tooltipPattern)) {
		...
		if (!jQuery.isArray(opts.colors)) {
			opts.colors = jQuery.parseData(opts.colors, true);
		}

		var colors = [];

		//a data object is set (no table)
		if (!jQuery.isEmptyObject(opts.data)) {

			//labels on the x axis are set
			if (opts.xlabels) {

				//convert them in the proper format
				opts.xlabels = jQuery.map(opts.xlabels, function (value, key) {
					return [[key, value]];
				});
				...
				opts.xlabels = function () {
					var ret = [];
					jQuery.each(opts.data, function (i, e) {
						jQuery.map(opts.data[i].data, function (value, key) {
							ret[value[0]] = key;
						});
					});
					return jQuery.map(ret, function (value, key) {
						return key;
					});
				}();
			...
			colors = jQuery.map(opts.data, function (value, key) {
				return opts.colors[key % opts.colors.length];
			});

			//data is from a table
		} else if (jQuery.isEmptyObject(opts.data) && $this.is('table')) {
			...
			default:
				//trigger an error
				jQuery.error('Orientation "' + opts.orientation + '" is not allowed');
			...
			jQuery.error('No data or data table!');
		...
		opts.flotobj = jQuery.extend({}, opts.flotobj, options);

		if (opts) jQuery.extend($this.data('wl_Chart'), opts);

		//let's draw the graph
		jQuery.fn.wl_Chart.methods.draw.call(this);
	});

};

jQuery.fn.wl_Chart.defaults = {
	width: null,
	height: 300,
	hideTable: true,
	data: {},
...
jQuery.fn.wl_Chart.version = '1.2';


jQuery.fn.wl_Chart.methods = {
	draw: function () {
		var $this = jQuery(this),
			_opts = $this.data('wl_Chart');
		//draw the chart and save it within the DOM
		$this.data('wl_Chart').plot = jQuery.plot(_opts.holder, _opts.data, _opts.flotobj);
	},
	set: function () {
		var $this = jQuery(this),
			options = {};
		...
		jQuery.each(options, function (key, value) {
			if (jQuery.fn.wl_Chart.defaults[key] !== undefined || jQuery.fn.wl_Chart.defaults[key] == null) {
